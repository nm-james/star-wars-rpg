...
ENT.Model = ENT.Model or "models/sally/tkaro/bx_commando_droid.mdl"
...
ENT.WeaponAttackDelay = ENT.WeaponAttackDelay or 0.2
...
function ENT:Initialize()
	...
	self.PassiveNPC = self.PassiveNPC or true
...
function ENT:OnTakeDamage( dmginfo )
	if ( not self.m_bApplyingDamage ) then
		...
		if attacker:IsPlayer() then
            ...
            local shouldLeap = math.random(1, 4)
...
function ENT:FireWeapon()
    ...
    bullet.Src = self:GetBonePosition(19)
    bullet.Dir = enem:LocalToWorld(enem:OBBCenter()) - self:GetBonePosition(19)
