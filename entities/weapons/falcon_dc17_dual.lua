...
function SWEP:PrimaryAttack()
    ...
    bullet.Src = self.Owner:GetAttachment(self.NextShotPos or 1).Pos
    ...
    if not self.NextShotPos or self.NextShotPos == 3 then
        self.NextShotPos = 1
    else
        self.NextShotPos = 3
    end

   self:SetNextPrimaryFire( CurTime() + 0.175 )
end
   ...
   function SWEP:DrawWorldModel()
      ...
      if not _Owner:GetNoDraw() then
         if _Owner:IsValid() and _Owner:Alive() then
            ...
            local offsetAng = Angle(175, 167, 0)
            ...
            local offsetVec = Vector(4, -1.75, 1.5)
            local offsetAng = Angle(0, -15, 0)
